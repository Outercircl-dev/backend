...
export class SupabaseJwtStrategy extends PassportStrategy(
  ...
  async validate(req: Request): Promise<any> {
    ...
    this.logger.debug(
      `Supabase JWT header present. tokenLength=${token?.length ?? 0} tokenPrefix=${token?.slice(0, 12) ?? ''}`,
    );
    ...
    try {
      ...
      this.logger.debug(
        `JWT Token verification completed. payload=${payloadPreview}`,
      );
      ...
      this.logger.error(
        `Supabase JWT verification error: ${message}`,
  ...
  private sanitizePayload(
    payload: SupabaseJwtPayload | unknown,
    ...
    if (input.app_metadata?.subscription_tier)
      sanitized.subscription_tier = input.app_metadata.subscription_tier;
  ...
  private extractSubscriptionTier(
    payload: SupabaseJwtPayload | Record<string, any> | unknown,
  ): MembershipTierKey | undefined {
    if (!payload || typeof payload !== 'object') {
      return undefined;
    }
